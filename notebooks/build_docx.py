...
para('These are SBA-guaranteed loans. The shape of the maturation curve transfers to unguaranteed '
     ...
     'gross loss — SBA’s own recovery tables put mature cohorts at 34–39% recovered, so a fund’s '
     'eventual loss is roughly a third lower than these numbers, though that is a bound and not a '
     'conversion: recovery is measured against a different amount and on a different year axis. '
     'Nothing here is causal: the case describes how vintages behave, not what moves them. And the '
     'projection assumes the loss timing of 1991–2014 still holds; if today’s borrowers fail on a '
     'different schedule, the method is wrong in a way it cannot detect on its own.')
para('The population itself is not in doubt. It reconciles against SBA’s published performance '
     'tables — a different reporting pipeline from the disclosure extract this case is built on — '
     'with a maximum deviation of four loans across nine fiscal years of 42,000 to 70,000 each, and '
     'exactly in three of them. Amounts sit 0.7–1.7% below the published figures in every single '
     'year, which the report’s own definition explains: it counts loan increases made after '
     'approval, and the extract does not carry them.')
